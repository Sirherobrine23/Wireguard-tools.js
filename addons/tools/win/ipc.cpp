...
#include <devguid.h>
#include <ndisguid.h>
#include <wireguard2.h>

static const DEVPROPKEY devpkey_name = { { 0x65726957, 0x7547, 0x7261, { 0x64, 0x4e, 0x61, 0x6d, 0x65, 0x4b, 0x65, 0x79 } }, 3 };

std::vector<std::string> kernel_get_wireguard_interfaces() {
  std::vector<std::string> list;
    ...
    dev_info_data.cbSize = sizeof(SP_DEVINFO_DATA);
		...
		if (CM_Get_DevNode_Status(&status, &problem_code, dev_info_data.DevInst, 0) == CR_SUCCESS && (status & (DN_DRIVER_LOADED | DN_STARTED)) == (DN_DRIVER_LOADED | DN_STARTED)) list.push_back(std::string(interface_name));
		...
		// if (SetupDiGetDeviceInstanceIdW(dev_info, &dev_info_data, NULL, 0, &buf_len) || GetLastError() != ERROR_INSUFFICIENT_BUFFER) continue;
...
// 		hashtable_entry *entry = hashtable_find_entry(&cached_kernel_interfaces, iface);
// 		if (entry) {
// 			DWORD buf_len;
//      if (CM_Get_Device_Interface_List_SizeW(&buf_len, (GUID *)&GUID_DEVINTERFACE_NET, (DEVINSTID_W)entry->value, CM_GET_DEVICE_INTERFACE_LIST_PRESENT) != CR_SUCCESS) goto err_hash;
// 			interfaces = calloc(buf_len, sizeof(*interfaces));
// 			if (!interfaces) goto err_hash;
// 			if (CM_Get_Device_Interface_ListW((GUID *)&GUID_DEVINTERFACE_NET, (DEVINSTID_W)entry->value, interfaces, buf_len, CM_GET_DEVICE_INTERFACE_LIST_PRESENT) != CR_SUCCESS || !interfaces[0]) {
// 				free(interfaces);
// 				interfaces = NULL;
// 				goto err_hash;
// 			}
// 			handle = CreateFileW(interfaces, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, 0, NULL);
// 			free(interfaces);
// 			if (handle == INVALID_HANDLE_VALUE) goto err_hash;
// 			return handle;
// err_hash:
// 			errno = EACCES;
// 			return NULL;
...
// 	}
    ...
    dev_info_data.cbSize = sizeof(SP_DEVINFO_DATA);
		...
		if (SetupDiGetDeviceInstanceIdW(dev_info, &dev_info_data, NULL, 0, &buf_len) || GetLastError() != ERROR_INSUFFICIENT_BUFFER) continue;
		buf = (WCHAR *)calloc(sizeof(*buf), buf_len);
		if (!buf) continue;
		if (!SetupDiGetDeviceInstanceIdW(dev_info, &dev_info_data, buf, buf_len, &buf_len)) goto cleanup_instance_id;
		if (CM_Get_Device_Interface_List_SizeW(&buf_len, (GUID *)&GUID_DEVINTERFACE_NET, (DEVINSTID_W)buf, CM_GET_DEVICE_INTERFACE_LIST_PRESENT) != CR_SUCCESS) goto cleanup_instance_id;
		interfaces = (WCHAR *)calloc(buf_len, sizeof(*interfaces));
		if (!interfaces) goto cleanup_instance_id;
		if (CM_Get_Device_Interface_ListW((GUID *)&GUID_DEVINTERFACE_NET, (DEVINSTID_W)buf, interfaces, buf_len, CM_GET_DEVICE_INTERFACE_LIST_PRESENT) != CR_SUCCESS || !interfaces[0]) {
			free(interfaces);
			interfaces = NULL;
...
// 		aip_count = 0;
// 		wg_aip = (void *)wg_peer + sizeof(WG_IOCTL_PEER);
// 		for ((aip) = (peer)->first_allowedip; (aip); (aip) = (aip)->next_allowedip) {

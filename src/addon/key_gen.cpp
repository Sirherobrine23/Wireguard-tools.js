...
namespace gereneate_Keys {
	Napi::Value presharedKey(const Napi::CallbackInfo& info) {
		...
		keyUtils::generatePreshared(interfacePresharedKey);
		wg_key_b64_string pskString;
		keyUtils::keyToBase64(pskString, interfacePresharedKey);
		return Napi::String::New(info.Env(), pskString);
	}

	Napi::Value privateKey(const Napi::CallbackInfo& info) {
		wg_key interfacePublicKey;
		keyUtils::generatePrivate(interfacePublicKey);
		wg_key_b64_string privString;
		keyUtils::keyToBase64(privString, interfacePublicKey);
	...
	Napi::Value publicKey(const Napi::CallbackInfo& info) {
		...
		keyUtils::keyFromBase64(interfacePrivateKey, privKey.Utf8Value().c_str());

		// Create public key
		wg_key interfacePublicKey;
		keyUtils::generatePublic(interfacePublicKey, interfacePrivateKey);

		// Convert to base64
		wg_key_b64_string pubString;
		keyUtils::keyToBase64(pubString, interfacePublicKey);
